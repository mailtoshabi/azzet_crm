...
class LoginController extends Controller
{
    ...
    */
    ...
    //      if (Auth::guard('executive')->attempt(array_merge($credentials, ['status' => 1]),)) {
    ...
    protected function validateLogin(Request $request)
    {
        ...
        if (filter_var($request->input('username'), FILTER_VALIDATE_EMAIL)) {
            // Validate as email
            $request->validate([
                'username' => 'required|email|exists:executives,email', // Check if email exists in users table
                'password' => 'required|string',
            ]);
        } else {
            // Validate as phone
            $request->validate([
                'username' => 'required|string|regex:/^[0-9]{10}$/|exists:executives,phone', // Regex for 10-digit phone numbers
                'password' => 'required|string',
            ]);
        }
    }
